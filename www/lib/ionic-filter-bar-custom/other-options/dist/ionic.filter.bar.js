...
(function (angular, document) {
  ...
  angular.module('jett.ionic.filter.bar')
    .directive('ionFilterBar', [
      ...
      '$state',
      '$rootScope',
      function ($timeout, $ionicGesture, $ionicPlatform, ServerConnector, $ionicPopup, $state, $rootScope) {
          ...
          filterBarTemplate =
            '<div class="filter-bar-wrapper filter-bar-{{::config.theme}} filter-bar-transition-{{::config.transition}}">' +
              '<div class="bar bar-header bar-{{::config.theme}} item-input-inset">' +
                ...
                '</label>' +
                '<button class="icon ion-search button button-clear" ng-class="getClearButtonClass()"></button>' +
              '</div>' +
            '</div>';
        } else {
          filterBarTemplate =
        ...
        return {
          ...
          link: function ($scope, $element) {
            ...
            var searchEl = el.querySelector('.ion-search');
            ...
            if ($scope.favoritesEnabled) {
              $scope.getClearButtonClass = function () {
                console.log("getClearButtonClass");
            ...
            var enableAutoSearch = function() {
              console.log("$scope.data.filterText", $scope.data.filterText);

              var search = ServerConnector.search($scope.data.filterText);
            ...
            searchEl.addEventListener('touchstart', enableAutoSearch);
            searchEl.addEventListener('mousedown', enableAutoSearch);
